...
export class ImageService implements IDataService {
        ...
        .then((carouselItems: ICarouselImage[]): void => {
          ...
          if (carouselItems != null) {
            for (i = 0; i < carouselItems.length; i++) {
              ...
              images.push('https://jpower4mvp.sharepoint.com/' + carouselItems[i].FileRef);
            }
          }
          resolve(images);
        });
  ...
  public readImages(listName?: string): Promise<ICarouselImage[]> {
    return new Promise<ICarouselImage[]>
      ((resolve: (itemId: ICarouselImage[]) => void, reject: (error: any) => void): void => {
